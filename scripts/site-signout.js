...
(function ($) {
    ...
    if(localStorage.signStatus==1){
        ...
        var sql_query_person1 = "SELECT * FROM visit_log WHERE activity=0 AND signout=0 AND project='"+localStorage.projectID+"' AND cast((strftime('%s','now')-strftime('%s',timestamp)) AS real)/60/60<12 GROUP BY firstname";
        DEMODB.transaction(function (tx) {
        ...
        if(localStorage.getItem("faceID") !== null){
            ...
            .complete(function(data){
                if(localStorage.personId==''){
                    ...
                    var sql_query_person2 = "SELECT * FROM visit_log WHERE personId='"+localStorage.personId+"' AND activity=0 AND signout=0 AND project='"+localStorage.projectID+"' AND cast((strftime('%s','now')-strftime('%s',timestamp)) AS real)/60/60<12 ORDER BY id DESC LIMIT 1";
                    DEMODB.transaction(function (tx) {
