            ...
            .complete(function(data){
                if(localStorage.personId==''){
                    ...
                    if(localStorage.signStatus==1){
                        ...
                        var sql_query_person = "SELECT * FROM visit_log WHERE personId='"+localStorage.personId+"' AND activity=1 AND signout=0 AND project='"+localStorage.projectID+"' AND cast((strftime('%s','now')-strftime('%s',timestamp)) AS real)/60/60<12 ORDER BY id DESC LIMIT 1";
                        DEMODB.transaction(function (tx) {
